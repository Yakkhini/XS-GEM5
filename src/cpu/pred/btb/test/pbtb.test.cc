...
class PBTBTest : public ::testing::Test
{
protected:
    void SetUp() override {
        ...
        pbtb = new DefaultBTB(16, 20, 4, 1, 1);
    }
...
TEST_F(PBTBTest, BasicPipelineOperation) {
    // Phase 1: First prediction
    boost::dynamic_bitset<> history(8, 0);
    std::vector<FullBTBPrediction> stagePreds(2);  // 2 stages
    // these fetch streams are predicted by a perfect predictor, no squash is needed, though we train pBTB in the process
    FetchStream streamA;
    ...
    pbtb->putPCHistory(streamA.startPC, history, stagePreds);
    // Get prediction metadata
    streamA.predMetas[0] = pbtb->getPredictionMeta();
    // second prediction
    pbtb->putPCHistory(streamB.startPC, history, stagePreds);
    streamB.predMetas[0] = pbtb->getPredictionMeta();
    ...
    pbtb->getAndSetNewBTBEntry(streamA);
    pbtb->update(streamA);
    pbtb->getAndSetNewBTBEntry(streamB);
    pbtb->update(streamB);
...
TEST_F(PBTBTest, MultipleBranchPipeline) {
    // Phase 1: Initial prediction
    Addr startAddr = 0x1000;
    boost::dynamic_bitset<> history(8, 0);
    std::vector<FullBTBPrediction> stagePreds(2);

    pbtb->putPCHistory(startAddr, history, stagePreds);
    auto meta = pbtb->getPredictionMeta();
    ...
    pbtb->getAndSetNewBTBEntry(stream);
    pbtb->update(stream);
...
TEST_F(PBTBTest, MultiCyclePipeline) {
    ...
    std::vector<FullBTBPrediction> stagePreds(2);
    pbtb->putPCHistory(addresses[0], history, stagePreds);
    auto meta = pbtb->getPredictionMeta();
    ...
    stagePreds.clear();
    stagePreds.resize(2);
    pbtb->putPCHistory(addresses[1], history, stagePreds);
    meta = pbtb->getPredictionMeta();
    ...
    stagePreds.clear();
    stagePreds.resize(2);
    pbtb->putPCHistory(addresses[2], history, stagePreds);
    meta = pbtb->getPredictionMeta();
    ...
    for (int i = 0; i < addresses.size(); i++) {
        stagePreds.clear();
        stagePreds.resize(2);
        pbtb->putPCHistory(addresses[i], history, stagePreds);
...
TEST_F(PBTBTest, ChangingTargetPipeline) {
    Addr startAddr = 0x1000;
    boost::dynamic_bitset<> history(8, 0);
    std::vector<FullBTBPrediction> stagePreds(2);
...
TEST_F(PBTBTest, MissingPreviousPC) {
    Addr startAddr = 0x1000;
    boost::dynamic_bitset<> history(8, 0);
    std::vector<FullBTBPrediction> stagePreds(2);
